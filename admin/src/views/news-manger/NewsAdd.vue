<template>
    <div>
        <el-page-header content="添加机车资讯" icon="" title="机车资讯管理" />
            ...
            <div>
                <el-form ref="MotoRuleFormRef" :model="MotoRuleForm" :rules="MotoRules" label-width="120px"
                    ...
                    <el-form-item label="内容" prop="content">
                        <Editor @changeEditor="onchangeEditor" />
                    </el-form-item>
                    ...
                    <el-form-item>
                        <el-button type="primary" @click="publish">添加资讯</el-button>
                    </el-form-item>
...
const publish = () => {
    MotoRuleFormRef.value.validate(async (vaid) => {
        if (vaid) {
            await upload("/adminapi/news/addnews", MotoRuleForm).then(res => {
                //判断是否添加成功 成功就跳转到机车资讯列表页
                if (res.status === 200) {
                    ElMessage.success("添加成功")
                    route.push("/news-manger/newsList")
                } else {
